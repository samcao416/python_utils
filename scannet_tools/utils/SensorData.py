...
class SensorData:
  ...
  def export_poses(self, output_path, scale=None, frame_skip=1):
    if not os.path.exists(output_path):
      os.makedirs(output_path)
    ...
    for f in range(0, len(self.frames), frame_skip):
      if scale is None:
        ...
        c2w = self.frames[f].camera_to_world
        c2w_list.append(c2w)
        #self.save_mat_to_file(c2w, os.path.join(output_path, str(f) + '.txt'))
    c2ws = np.array(c2w_list)
    np.save(os.path.join(output_path,'scale.npy'),scale)
    np.save(os.path.join(output_path,'poses.npy'),c2ws)
  ...
  def process_camera_dict2(self, output_path, scale_matrix):

    #_, scale_matrix = self.get_scale_mat(mesh)

    out_dict = {}
    instrinsic_mat_d = self.intrinsic_depth
    instrinsic_mat_c = self.intrinsic_color
    # scale pixels to [-1, 1]
    mask_camera = []
    for f in range(0, len(self.frames), 1):
      out_dict['intri_d_mat_%d' % f] = instrinsic_mat_d.astype(np.float32)
      out_dict['intri_c_mat_%d' % f] = instrinsic_mat_c.astype(np.float32)
      world_mat_inv = self.frames[f].camera_to_world
      if np.any(np.isnan(world_mat_inv)) or np.any(np.isinf(world_mat_inv)):
          logging.warning('inf world mat for %s: %d' % (output_path, f))
          print('invalid world matrix!')
      ...
      out_dict['c2w%d' % f] = world_mat_inv.astype(np.float32)
      out_dict['scale_mat_%d' % f] = scale_matrix.astype(np.float32)
      out_dict['camera_mask'] = mask_camera
    out_file = os.path.join(output_path, 'cameras.npz')
    np.savez(out_file, **out_dict)